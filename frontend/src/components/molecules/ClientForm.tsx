...
const ClientForm: React.FC<{ type: 'create' | 'edit' }> = ({ type }) => {
  ...
  const [formErrorMessages, setFormErrorMessages] = useState<{
    [key in keyof IClientCreateData]?: string;
  }>({});
  const { chosenClient, clearChosenClient } = useClientStore();
  ...
  const handleSubmit = () => {
    ...
    const fields: (keyof IClientCreateData)[] = [
      'name',
      'email',
      'cpf',
      'cellphone',
    ...
    const activeErrors = Object.values(updatedErrors).filter(
      (error) => error !== ''
    );
    ...
    if (type === 'create') {
      ...
      const filteredData: IClientEditData = Object.entries(formData).reduce(
        (acc, [key, value]) => {
          if (value !== '') {
  ...
  return (
    ...
    >
      <ClientDasboardTitle />
      <Divider />
      <Box className="w-full h-[50px] flex justify-between items-center my-5">
        <Box>
          <Typography
            sx={{ fontWeight: '600', fontSize: '17px', color: '#737980' }}
          >
            {type === 'create' ? 'Novo' : 'Editar'} usuário
          </Typography>
          <Typography sx={{ color: '#949494', fontSize: '15px' }}>
            {type === 'create' ? createUserMessage : editUserMessage}
          </Typography>
        </Box>
      </Box>
      <Box
