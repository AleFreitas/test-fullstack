...
const ClientListItem: React.FC<IClientListItemProps> = ({ client }) => {
  const { name, email, status, cellphone, cpf } = client;
  ...
  const { setChosenClient } = useClientStore();
  const navigate = useNavigate();

  const upperLineTextStyle = {
    fontWeight: '600',
    fontSize: '15px',
    color: '#737980',
  };
  ...
  return (
    ...
    >
      ...
      <Box className="w-1/4 flex items-center">
        <Box
          sx={{
            backgroundColor: `${statusInfo[status][0]}`,
            width: '10px',
            height: '10px',
            borderRadius: '50%',
          }}
        />
        <Typography
          sx={{ marginLeft: '5px', color: '#949494', fontSize: '15px' }}
        >
          {statusInfo[status][1]}
        </Typography>
      </Box>
